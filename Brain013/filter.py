...
# -------------------------* Lire les paramétres du client *---------------------#
def get_formated_data(formated , delimiter):
    data = pd.read_csv(formated  , sep=delimiter)
    entries = data[params]
...
def write_on_csv(path, d , delimiter):
    d = pd.DataFrame(d)
    d = d.fillna(0)
    d.to_csv(path,index=False, sep=delimiter , header=False)


def filter_datas(f , entry , out , delimiter):
    print(f)
    print(entry)
    inputs, output = get_formated_data(f , delimiter)
    print(inputs)
    write_on_csv(entry,inputs , delimiter)
    write_on_csv(out, output , delimiter)
