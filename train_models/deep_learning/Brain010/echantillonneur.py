...
def add_dateTime(path) :
    ...
    data.drop(columns=['date_time' , 'DateTime'], inplace=True)
...
def echantillonner(path , time, path_save) :
    ...
    print(final_data)
    pd.DataFrame(final_data).to_csv(path_save, sep=';')

#%%

echantillonner("Brain010/data_raw/to_sample/data_train.csv" , 5 , "Brain010/data_raw/train/data_train.csv")
